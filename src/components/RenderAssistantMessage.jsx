...
const RenderAssistantMessage = ({ children }) => {
  ...
  if (typeof content === "string") {
    return <div className="prose prose-sm max-w-none text-gray-800 leading-relaxed whitespace-pre-wrap">{content}</div>;
  }
  ...
  const renderNumberedList = (items, title = null) => {
    ...
    return (
      <div className="space-y-2">
        {title && <h4 className="font-medium text-gray-800 flex items-center gap-2 mb-2">{title}</h4>}
        <div className="space-y-2">
  ...
  const renderModuleItem = (module, idx) => {
    ...
    return (
      <AccordionItem key={idx} value={`module-${idx}`} className="border border-gray-200 rounded-lg overflow-hidden">
        <AccordionTrigger className="hover:bg-gray-50 px-4 py-3 text-left border-b border-gray-200">
          <div className="flex items-center justify-between w-full">
            ...
            <div className="text-right text-xs space-y-1">
              <div className="text-gray-600">{duration} mo</div>
        ...
        <AccordionContent className="px-4 py-4 space-y-4 bg-gray-50">
          <div className="grid md:grid-cols-2 gap-4 text-sm">
            ...
            <div className="space-y-3">
              <div>
                <p className="font-medium text-gray-700 mb-1">Format & Time</p>
              ...
              {prerequisites.length > 0 && (
                <div>
                  <p className="font-medium text-gray-700 mb-1">Prerequisites</p>
                  <div className="flex flex-wrap gap-1">
          ...
          {skills.length > 0 && (
            <div>
              <p className="font-medium text-gray-700 mb-2">Skills Gained</p>
          ...
          {milestones.length > 0 && (
            <div>
              <p className="font-medium text-gray-700 mb-2">Key Milestones</p>
              <div className="space-y-2">
                {milestones.slice(0, 4).map((milestone, mIdx) => (
                  <div key={mIdx} className="flex items-start gap-2 p-2 bg-white rounded text-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t border-gray-200">
            {assessment && (
              <div className="bg-blue-50 p-3 rounded">
                <p className="font-medium text-blue-800 text-xs mb-1">Assessment</p>
                <p className="text-blue-700 text-xs">{assessment}</p>
              </div>
            )}
            {certification && (
              <div className="bg-green-50 p-3 rounded">
                <p className="font-medium text-green-800 text-xs mb-1">Certification</p>
                <p className="text-green-700 text-xs">{certification}</p>
              </div>
            )}
          </div>
        </AccordionContent>
      ...
      <Card key={idx} className="border-gray-200 hover:shadow-md transition-shadow">
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-3">
            <h4 className="font-semibold text-gray-800 pr-2 flex-1">{name}</h4>
  ...
  const renderQuestionItem = (question, idx) => {
    ...
    return (
      <Card key={idx} className="border-gray-200 hover:shadow-sm">
        <CardHeader className="p-4 pb-3">
          <div className="flex items-start justify-between gap-3">
            <div className="flex-1">
              <h4 className="font-semibold text-gray-800 text-sm leading-tight">{qText}</h4>
            </div>
            <div className="flex flex-col items-end gap-2 text-xs min-w-[80px]">
        ...
        <CardContent className="p-4 pt-0 space-y-3">
          {answer && (
            <div className="bg-gray-50 p-3 rounded-lg">
              <p className="font-medium text-gray-700 mb-2 text-xs">Suggested Answer:</p>
              <p className="text-sm text-gray-600 leading-relaxed">{answer}</p>
              {length && (
                <p className="text-xs text-gray-500 mt-2 font-mono bg-white px-2 py-1 rounded inline-block">
                  📝 {length}
                </p>
              )}
            </div>
          ...
          {followups.length > 0 && (
            <div className="bg-blue-50 p-3 rounded-lg border-l-4 border-blue-400">
              <p className="font-medium text-blue-800 mb-2 flex items-center gap-2 text-sm">
                <AlertTriangle size={14} /> Follow-up Questions
              </p>
              <div className="space-y-1">
                {followups.map((fu, fuIdx) => (
                  <div key={fuIdx} className="flex items-start gap-2 text-sm text-blue-700">
  ...
  switch (response.response_type) {
    case "text":
      return (
        <div className="prose prose-sm max-w-none">
          <p className="text-gray-800 leading-relaxed">{response.text_response}</p>
        </div>
      );
    ...
    case "analysis":
      if (!response.career_advisor_report) {
        return (
          <div>
            <p className="font-semibold text-gray-800 text-lg mb-4">{response.text_response}</p>
      ...
      return (
        <div className="space-y-6">
          ...
          {modules.length > 0 || projects.length > 0 && (
            <Card className="border-gray-200">
              ...
              <CardContent className="space-y-6">
                {/* Overview Stats */}
                <div className="grid md:grid-cols-4 gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
                  <div className="text-center">
                    <p className="text-2xl font-bold text-indigo-600">{targetRole}</p>
                    <p className="text-xs text-gray-600 mt-1">Target Role</p>
                  </div>
                  <div className="text-center">
                ...
                {modules.length > 0 && (
                  <div className="space-y-3">
                    <h3 className="text-lg font-semibold text-indigo-800 flex items-center gap-2 border-b border-indigo-200 pb-2">
          ...
          {Object.keys(careerInsights).length > 0 && (
            <Card className="border-gray-200">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="text-center p-4 bg-white rounded-lg shadow-sm border">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <span className="text-lg font-bold text-green-600 break-words">
                  ...
                  <div className="text-center p-4 bg-white rounded-lg shadow-sm border">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <span className="text-sm font-medium text-blue-600 break-words">
                        {safeGet(careerInsights, 'location_insights', 'Global').substring(0, 25)}
                        {safeGet(careerInsights, 'location_insights', '').length > 25 ? '...' : ''}
                      </span>
                    </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    {safeArray(safeGet(careerInsights, 'industry_trends')).length > 0 && (
                      <div>
                        <h4 className="font-semibold text-green-800 mb-2 flex items-center gap-2">
                          <TrendingUp size={16} className="text-blue-600" /> Trends
                        </h4>
                    ...
                    {safeArray(safeGet(careerInsights, 'top_skills_in_demand')).length > 0 && (
                      <div>
                        <h4 className="font-semibold text-green-800 mb-2 flex items-center gap-2">
                          <Brain size={16} className="text-purple-600" /> In-Demand Skills
                        </h4>
            ...
            <Card className="border-gray-200">
              ...
              <CardContent className="space-y-6">
                ...
                {safeArray(safeGet(jobPrep, 'common_interview_questions')).length > 0 && (
                  <div className="space-y-4">
                    <h3 className="text-lg font-semibold text-orange-800 flex items-center gap-2 border-b border-orange-200 pb-2">
                    ...
                    <div>
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {safeArray(safeGet(jobPrep, 'resources')).slice(0, 6).map((resource, idx) => (
                          <div key={idx} className="p-3 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                            <p className="text-sm text-gray-700 line-clamp-2">{String(resource)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {safeGet(jobPrep, 'recommended_study_time') && (
                  <div className="grid md:grid-cols-3 gap-6 pt-4 border-t border-orange-200">
                    <div className="bg-gradient-to-br from-orange-50 to-red-50 p-4 rounded-lg">
                      ...
                      {safeGet(jobPrep, 'behavioral_vs_technical_ratio') && (
                        <div className="text-xs space-y-1 mt-3 pt-2 border-t border-orange-200">
                          <p className="font-medium text-orange-800">Interview Focus:</p>
                          <p className="text-orange-700">{safeGet(jobPrep, 'behavioral_vs_technical_ratio')}</p>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="bg-red-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm max-h-32 overflow-y-auto">
                          {safeArray(safeGet(jobPrep, 'common_pitfalls')).slice(0, 4).map((pitfall, idx) => (
                            <div key={idx} className="flex items-start gap-2 p-2 bg-white rounded text-red-700">
                      ...
                      <div className="bg-blue-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2 text-sm max-h-32 overflow-y-auto">
                          {safeArray(safeGet(jobPrep, 'role_specific_scenarios')).slice(0, 3).map((scenario, idx) => (
                            <div key={idx} className="p-2 bg-white rounded text-blue-700 text-xs italic border-l-2 border-blue-300">
                              {String(scenario)}
                            </div>
                          ))}
                        </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      {safeArray(safeGet(jobPrep, 'hr_culture_insights')).slice(0, 6).map((insight, idx) => (
                        <div key={idx} className="p-3 bg-orange-50 rounded-lg border border-orange-200">
                          <p className="text-sm text-orange-800 leading-relaxed">{String(insight)}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-sm">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-xs text-gray-600 uppercase tracking-wide">Performance</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {/* Empty State */}
          {modules.length === 0 && projects.length === 0 && Object.keys(careerInsights).length === 0 && Object.keys(jobPrep).length === 0 && (
            <Card className="border-gray-200">
    ...
    default:
      ...
      return (
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          ...
          <p className="text-sm text-yellow-700 mb-3">Unknown response format detected:</p>
          <div className="bg-white p-3 rounded border overflow-x-auto">
            <pre className="text-xs text-yellow-800 whitespace-pre-wrap max-h-60 overflow-y-auto">
